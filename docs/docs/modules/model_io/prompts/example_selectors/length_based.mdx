# Select by length | 長さによって選択

この例示セレクターは、長さに基づいて使用する例を選択します。これは、コンテキストウィンドウの長さを超えてしまうプロンプトを構築することを懸念している場合に便利です。長い入力に対しては、含める例を少なく選択し、短い入力に対しては、より多くの例を選択します。
