# Moderation chain | モデレーションチェーン

このノートブックでは、モデレーションチェーンの使用例と、それを行ういくつかの一般的な方法を紹介します。モデレーションチェーンは、憎悪的、暴力的などのテキストを検出するのに役立ちます。これは、ユーザー入力だけでなく、言語モデルの出力にも適用すると便利です。OpenAIのような一部のAPIプロバイダーは、あなたやエンドユーザーが一部の有害なコンテンツを生成することを[特に禁止しています](https://beta.openai.com/docs/usage-policies/use-case-policy)。これに準拠するため（そして一般的にあなたのアプリケーションが有害でないようにするために）、LLMChainsにモデレーションチェーンを追加することがよくあります。これにより、LLMが生成するあらゆる出力が有害でないことを確認できます。
...
モデレーションチェーンに渡されたコンテンツが有害な場合、それを処理する最善の方法は一つに限られません。処理方法はお使いのアプリケーションによって異なるでしょう。時には、チェーンでエラーを発生させて（そしてそのエラーをアプリケーションで処理することもあります）。他の時には、テキストが有害であったことをユーザーに説明する何らかのメッセージを返すことを望むかもしれません。他にも処理方法はあるかもしれません。このウォークスルーでは、これらの方法をすべてカバーします。
...
1. 任意のテキストをモデレーションチェーンを通じて実行する方法。
   > How to run any piece of text through a moderation chain.
2. LLMChainにモデレーションチェーンを追加する方法。
...
## How to use the moderation chain | モデレーションチェーンの使用方法

こちらは、デフォルト設定を使用したモデレーションチェーンの使用例です（フラグが立てられた内容について説明する文字列が返されます）。
...
こちらは、モデレーションチェーンを使用してエラーを発生させる例です。
...
## How to create a custom Moderation chain | カスタムモデレーションチェーンの作成方法

カスタムエラーメッセージを含むカスタムモデレーションチェーンの作成例をこちらでご紹介します。これにはOpenAIのモデレーションエンドポイントの結果に関するある程度の知識が必要です。詳細は[こちらのドキュメント](https://beta.openai.com/docs/api-reference/moderations)をご覧ください。
...
## How to append a Moderation chain to an LLMChain | LLMChainにモデレーションチェーンを追加する方法

LLMChainとモデレーションチェーンを簡単に組み合わせるためには、`SequentialChain` 抽象化を使用することができます。

> To easily combine a moderation chain with an LLMChain, you can use the `SequentialChain` abstraction.

まずは、`LLMChain`が単一の入力のみを持つ簡単な例から始めましょう。この目的のために、モデルにプロンプトを送り、何か有害なことを言わせるようにします。
...
それでは、複数の入力を持つLLMChainを使用した例を見ていきましょう（SimpleSequentialChainを使えないため、少し複雑です）
