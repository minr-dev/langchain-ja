...
> [Markdown](https://ja.wikipedia.org/wiki/Markdown)は、プレーンテキストエディタを使用してフォーマットされたテキストを作成するための軽量マークアップ言語です。
>
> > [Markdown](https://en.wikipedia.org/wiki/Markdown) is a lightweight markup language for creating formatted text using a plain-text editor.

`Markdown`ドキュメントを、後続の処理で使用できるドキュメント形式に読み込む方法について説明します。
...
内部的に、Unstructuredは異なるテキストのチャンクごとに「要素」を作成します。デフォルトではこれらを組み合わせていますが、`mode="elements"`を指定することで、それらを簡単に分けて保持することが可能です。
