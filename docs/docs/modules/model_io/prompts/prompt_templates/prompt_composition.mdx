...
このノートブックでは、複数のプロンプトを組み合わせる方法について説明しています。これは、プロンプトの一部を再利用したいときに便利です。これはPipelinePromptを用いて実現することができます。PipelinePromptは主に2つの主要な部分から構成されています：
...
* パイプラインプロンプト：文字列の名前とプロンプトテンプレートからなるタプルのリストです。各プロンプトテンプレートはフォーマットされ、その後、同じ名前の変数として次のプロンプトテンプレートに渡されます。
