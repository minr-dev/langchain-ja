# Replicating MRKL | MRKLの複製

このウォークスルーは、エージェントを使用して[MRKL](https://arxiv.org/pdf/2205.00445.pdf)システムを複製する方法を示しています。

> This walkthrough demonstrates how to replicate the [MRKL](https://arxiv.org/pdf/2205.00445.pdf) system using agents.

これは例としてChinookデータベースを使用します。セットアップするには、https://database.guide/2-sample-databases-sqlite/ の指示に従って、`.db`ファイルをこのリポジトリのルートにある「notebooks」フォルダに配置してください。
