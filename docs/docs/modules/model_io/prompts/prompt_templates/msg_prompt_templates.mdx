# Types of `MessagePromptTemplate` | `MessagePromptTemplate`のタイプ

LangChainは、様々なタイプの`MessagePromptTemplate`を提供しています。最も一般的に使用されるのは`AIMessagePromptTemplate`、`SystemMessagePromptTemplate`、そして`HumanMessagePromptTemplate`で、それぞれAIのメッセージ、システムのメッセージ、人間のメッセージを生成します。
...
しかし、チャットモデルが任意の役割を持つチャットメッセージを取り扱うことをサポートしている場合、ユーザーが役割名を指定できる`ChatMessagePromptTemplate`を使用できます。
...
LangChainは`MessagesPlaceholder`も提供しており、フォーマット中にどのメッセージをレンダリングするかを完全に制御することができます。これは、メッセージプロンプトテンプレートでどの役割を使用すべきかが不確かな場合や、フォーマット中にメッセージリストを挿入したい場合に有用です。
