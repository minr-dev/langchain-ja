...
このページでは、langchain上に構築されたLLMアプリを評価し、追跡する方法について、[TruLens](https://trulens.org)を使用する方法を説明しています。

> This page covers how to use [TruLens](https://trulens.org) to evaluate and track LLM apps built on langchain.

## What is TruLens? | TruLensとは何ですか？

TruLensは、大規模言語モデル（LLM）ベースのアプリケーション向けに計測と評価ツールを提供する[オープンソース](https://github.com/truera/trulens)パッケージです。
...
LLMチェーンを作成したら、評価と追跡のためにTruLensを使用できます。TruLensには、多数の[すぐに使えるフィードバック機能](https://www.trulens.org/trulens_eval/feedback_functions/)があり、またLLM評価のための拡張性のあるフレームワークでもあります。
...
LLMを評価するためのフィードバック機能を設定した後、TruChainを使用してアプリケーションをラップすることで、LLMアプリの詳細なトレーシング、ログ記録、および評価を取得できます。
...
これで、あなたのLLMベースのアプリケーションを探索できるようになりました！

> Now you can explore your LLM-based application!

これを行うことで、LLMアプリケーションのパフォーマンスが一目でどのように動いているかを理解するのに役立ちます。LLMアプリケーションの新しいバージョンを繰り返し作成する際には、設定したさまざまな品質メトリクスを通じて、それらのパフォーマンスを比較することができます。また、レコードレベルでの評価を閲覧するだけでなく、各レコードのチェーンメタデータを探索することもできます。
...
TruLensに関する詳細は、[trulens.org](https://www.trulens.org/)をご覧ください。
