...
> [Portable Document Format (PDF)](https://ja.wikipedia.org/wiki/PDF)は、ISO 32000として標準化されたファイル形式で、アドビが1992年に文書を提示するため、テキストの書式設定や画像を含む形で開発しました。これはアプリケーションソフトウェア、ハードウェア、およびオペレーティングシステムに依存しない方法で表示されるようになっています。
>
> > [Portable Document Format (PDF)](https://en.wikipedia.org/wiki/PDF), standardized as ISO 32000, is a file format developed by Adobe in 1992 to present documents, including text formatting and images, in a manner independent of application software, hardware, and operating systems.

これは、下流のプロセスで使用するドキュメント形式に`PDF`ドキュメントを読み込む方法について説明しています。

> This covers how to load `PDF` documents into the Document format that we use downstream.

## Using PyPDF | PyPDFの使用

`pypdf`を使用してPDFをドキュメントの配列として読み込み、各ドキュメントには`page`番号を含むページ内容とメタデータが含まれます。
...
このアプローチの利点は、文書をページ番号付きで取得できることです。
...
### Extracting images | 画像の抽出

`rapidocr-onnxruntime`パッケージを使用することで、画像をテキストとしても抽出できます。
...
## Using MathPix | MathPixの使用

Daniel Grossの<https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21>に触発されて
...
内部的には、Unstructuredは異なるテキストのチャンクごとに異なる「要素」を作成します。デフォルトではこれらを結合しますが、`mode="elements"`を指定することによって、これらの要素を簡単に分離したまま保持することができます。
...
### Fetching remote PDFs using Unstructured | Unstructuredを使用してリモートPDFを取得する

これは、オンライン上のPDFを、後続の処理で使用できるドキュメント形式に読み込む方法について説明しています。この方法は、https://open.umn.edu/opentextbooks/textbooks/ や https://arxiv.org/archive/ など、様々なオンラインPDFサイトに利用できます。

> This covers how to load online PDFs into a document format that we can use downstream. This can be used for various online PDF sites such as https://open.umn.edu/opentextbooks/textbooks/ and https://arxiv.org/archive/

注意：他のPDFローダーもリモートPDFの取得に使用できますが、`OnlinePDFLoader`はレガシーな機能であり、`UnstructuredPDFLoader`専用に動作するように作られています。
...
### Using PDFMiner to generate HTML text | PDFMinerを使用してHTMLテキストを生成する

この機能は、出力されたHTMLコンテンツを`BeautifulSoup`で解析することで、フォントサイズ、ページ番号、PDFのヘッダーやフッターなどの、より構造化されたり詳細な情報を得るために、テキストを意味的にセクションに分けるのに役立ちます。
...
## Using PyMuPDF | PyMuPDFの使用

これはPDF解析オプションの中で最も高速であり、PDFとそのページに関する詳細なメタデータを含んでおり、1ページごとに1つのドキュメントを返します。
...
さらに、`load`コール時にキーワード引数として[PyMuPDFドキュメント](https://pymupdf.readthedocs.io/en/latest/app1.html#plain-text/)のオプションを渡すことができ、それらは`get_text()`コールへと引き継がれます。
...
## Using PDFPlumber | PDFPlumberの使用

PyMuPDFのように、出力されるドキュメントにはPDFとそのページに関する詳細なメタデータが含まれており、1ページごとに1つのドキュメントが返されます。
...
## Using AmazonTextractPDFParser | AmazonTextractPDFParserの使用

AmazonTextractPDFLoaderは、PDFをドキュメント構造に変換するために[Amazon Textract Service](https://aws.amazon.com/textract/)を呼び出します。このローダーは現在、純粋なOCR機能のみを提供していますが、需要に応じてレイアウトサポートなどの追加機能を導入する予定です。単一ページおよび複数ページのドキュメントに対応しており、最大3000ページ、512MBのサイズまで扱えます。
...
呼び出しが成功するためには、[AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html)の要件と同様、AWSアカウントが必要です。
...
AWSの設定を除けば、他のPDFローダーと非常によく似ており、JPEG、PNG、TIFFといった非ネイティブPDFフォーマットもサポートしています。
