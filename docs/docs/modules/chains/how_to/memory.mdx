# Adding memory (state) | メモリ（ステート）の追加

チェーンはMemoryオブジェクトを使って初期化することができ、これによりチェーンの呼び出し間でデータが保持されます。これにより、チェーンは状態を持つことになります。
...
基本的に、`BaseMemory`は`langchain`がメモリをどのように保存するかのインターフェースを定義しています。これにより、`load_memory_variables`メソッドを通じて保存されたデータを読み込むことと、`save_context`メソッドを通じて新しいデータを保存することが可能になります。詳細については[メモリ](/docs/modules/memory/)セクションで学ぶことができます。
