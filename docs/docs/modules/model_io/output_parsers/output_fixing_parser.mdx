...
この出力パーサーは別の出力パーサーをラップしており、最初のパーサーが失敗した場合には、別のLLMを呼び出してエラーを修正します。

> This output parser wraps another output parser, and in the event that the first one fails it calls out to another LLM to fix any errors.

しかし、エラーを投げる以外にもできることがあります。具体的には、フォーマットされていない出力とフォーマットの指示をモデルに渡し、それを修正してもらうことができます。

> But we can do other things besides throw errors. Specifically, we can pass the misformatted output, along with the formatted instructions, to the model and ask it to fix it.

この例では、上記のPydantic出力パーサーを使用します。スキーマに準拠していない結果を渡すとどうなるか見てみましょう：
...
これで、`OutputFixingParser`を構築して使用することができます。この出力パーサーは、別の出力パーサーを引数として取るだけではなく、フォーマットの誤りを修正するために試みるLLMも必要です。
