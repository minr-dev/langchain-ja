...
これは、ディレクトリ内のすべてのドキュメントを読み込む方法を説明しています。

> This covers how to load all documents in a directory.

内部では、デフォルトで[UnstructuredLoader](/docs/integrations/document_loaders/unstructured_file)が使用されています。
...
`glob`パラメータを使用して、どのファイルを読み込むかを制御できます。ここで、`.rst`ファイルや`.html`ファイルは読み込まれないことに注意してください。
...
## Show a progress bar | 進捗バーを表示する

デフォルトではプログレスバーは表示されません。プログレスバーを表示するには、`tqdm`ライブラリをインストール（例：`pip install tqdm`）し、`show_progress`パラメータを`True`に設定してください。
...
## Use multithreading | マルチスレッディングを使用する

デフォルトでは、ローディングは1つのスレッドで行われます。複数のスレッドを利用するためには、`use_multithreading` フラグを true に設定してください。
...
## Change loader class | ローダークラスを変更する

デフォルトでは、`UnstructuredLoader` クラスが使用されます。しかし、ローダーの種類は非常に簡単に変更できます。
...
Pythonのソースコードファイルを読み込む必要がある場合は、`PythonLoader`を使用してください。
...
## Auto-detect file encodings with TextLoader | TextLoaderを使用してファイルのエンコーディングを自動検出する

この例では、`TextLoader` クラスを使用してディレクトリから大量の任意のファイルを読み込む際に役立ついくつかの戦略を見ていきます。
...
問題を説明するために、まずは任意のエンコーディングで複数のテキストを読み込むことを試みましょう。
...
`example-non-utf8.txt` ファイルは異なるエンコーディングを使用しているため、`load()` 関数はデコードに失敗したファイルを示す役立つメッセージを伴って失敗します。
...
`TextLoader`のデフォルトの挙動では、いずれかのドキュメントの読み込みに失敗すると、全体の読み込みプロセスが失敗し、どのドキュメントも読み込まれなくなります。

> With the default behavior of `TextLoader` any failure to load any of the documents will fail the whole loading process and no documents are loaded.

### B. Silent fail | B. サイレントフェイル

`DirectoryLoader`に`silent_errors`パラメータを渡すことで、読み込めなかったファイルをスキップして読み込み処理を続行することができます。
...
### C. Auto detect encodings | C. エンコーディングの自動検出

`TextLoader`に`autodetect_encoding`を渡すことで、失敗する前にファイルのエンコーディングを自動検出するよう依頼することもできます。
